...
export default function FeaturedCollections() {
  ...
  const res = collections.nodes.findIndex(
  ...
  return (
    <section className="w-full gap-4 md:gap-8 grid sm:p-4 md:p-8 lg:p-12">
      <h2 className="whitespace-pre-wrap max-w-prose font-bold text-lead p-4">
        Collections
      </h2>
          ...
          return (
            <Link key={collection.id} to={`/collections/${collection.handle}`}>
              <div className="grid gap-4">
                {collection?.image && (
                  <Image
                    className="rounded shadow-border overflow-clip inline-block object-cover"
                    width={"100%"}
                    height={336}
                ...
                <h2 className="whitespace-pre-wrap max-w-prose font-medium text-copy pl-3">
                  {collection.title}
                </h2>
              </div>
            </Link>
...
const QUERY = gql`
  query FeaturedCollections {
    collections(first: 33, sortKey: UPDATED_AT) {
      nodes {
        id
        title
        handle
